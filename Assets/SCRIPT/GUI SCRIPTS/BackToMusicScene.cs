...
public class BackToMusicScene : MonoBehaviour {
			...
			if (Input.GetButtonDown("Fire1"))
			{
				...
				if (Physics.Raycast(userTouchRayM, out raycast_infoM, raycast_range))
				{
					...
					if (raycast_infoM.collider.gameObject == this.gameObject)
					{
						...
						//GameObject.Find ("BackgroundMusicHolder").GetComponentInChildren<AudioSource>().enabled = true;
						Destroy (GameObject.Find ("sounds_holder"));
